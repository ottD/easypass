...
export function getState()
{
  ...
  return storage.has(userSaltKey).then(value =>
  {
...
function _setUserid(masterPassword)
{
  ...
  return storage.get(userSaltKey, null).then((rawUserSalt) =>
  {
    if (rawUserSalt == null)
    {
      rawUserSalt = generateRandom(16);
      return storage.set(userSaltKey, rawUserSalt, null).then(() => importHmacSecret(rawUserSalt));
    }
